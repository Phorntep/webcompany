                ...
                If txtpassword.Text.Length >= 4 Then
                    If txtpassword.Text = txtconfirmPass.Text Then
                        Try
                            ...
                            Dim sql As String = "insert into User_bill values('" & txtusername.Text & "','" & Encrypt(txtpassword.Text, key) & "','" & txtCompany.Text & "','" & txtSupplier.Text & "',1,GETDATE(),'" & txtaddress.Text & "','" & txtPhone.Text & "'," & ddLevel.SelectedValue & ",'" & ddLevel.SelectedItem.Text & "',null,null)"
                            Dim ap As SqlDataAdapter = New SqlDataAdapter(sql, connection)
                            ap.Fill(ds)
